    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-gray-600 max-w-md">
          The ebook you are looking for might have been removed or is
          ...
          <div className="relative group w-full aspect-3/4 bg-[#1A4B58] rounded-lg shadow-md overflow-hidden flex items-center justify-center border border-[#E0DCD3]">
            {coverImage ? (
              ...
              <div className="text-center p-6 text-white/80">
                ...
                <Link
                  href={`/writers/${bookData?.writerId}`}
                  className="text-sm text-[#1A4B58] font-semibold  mb-6 cursor-pointer hover:underline transition-all"
                >
                  by {writerName}
                </Link>
              </div>
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 py-3 px-4 rounded-md border text-sm font-medium transition-all ${
              isBookmarked
                ...
                : "bg-white border-[#E0DCD3] text-gray-700"
            }`}
          >
            <Bookmark
              className={`w-4 h-4 ${isBookmarked ? "fill-gray-800" : ""}`}
            />
            {isBookmarked ? "Bookmarked" : "Bookmark"}
          </button>

          <div className="bg-[#FAF9F5] rounded-lg p-5 border border-[#EAE6DF]">
            <span className="text-[10px] tracking-wider uppercase font-semibold text-gray-400 block mb-2">
              About The Writer
            </span>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <Link
                  href={`/writers/${bookData?.writerId}`}
                  className="font-serif font-bold cursor-pointer text-gray-900 block hover:text-[#1A4B58] hover:underline transition-all"
                >
                  {writerName}
                </Link>
                <p className="text-xs text-gray-500">
                  {writer?.email || "Verified Creator"}
                </p>
              </div>
            </div>
            <p className="text-xs text-gray-600 leading-relaxed">
              Official writer profile registered on Fable platform.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex flex-col justify-between space-y-6">
          <div>
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className="px-2.5 py-0.5 bg-[#EAE6DF] text-gray-700 text-xs font-semibold rounded uppercase tracking-wider">
                {category}
              </span>
              <span
                className={`px-2.5 py-0.5 text-xs font-semibold rounded uppercase tracking-wider ${isSoldOut ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
              >
                {status}
              </span>
            ...
            <h1 className="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-1 leading-tight">
              {title}
            </h1>
            <p className="text-sm text-gray-500 mb-6">
              by{" "}
              <span className="text-[#1A4B58] font-medium">{writerName}</span>
            </p>
            ...
            <div className="bg-white border border-[#EAE6DF] rounded-lg p-5 mb-6 shadow-xs">
              ...
              <div className="mt-4">
                {hasPurchased ? (
                  ...
                  <button
                    disabled
                    className="w-full bg-gray-200 text-gray-400 py-3 px-4 rounded-md font-medium text-sm cursor-not-allowed"
                  >
                  ...
                  >
                    ...
                    {currentUser ? (
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          alert("Please log in to purchase this book.")
                        }
            ...
            <div className="mb-8">
              <h3 className="text-[10px] tracking-wider uppercase font-semibold text-gray-400 mb-2">
                About This Ebook
              </h3>
              ...
              {!hasPurchased && !isWriter && (
                <div className="mt-4 bg-linear-to-b from-transparent to-[#FAF9F5] p-6 rounded-lg text-center border border-dashed border-[#E0DCD3]">
                  ...
                  <p className="text-xs text-gray-500 font-medium">
                    Full book contents are hidden until purchase.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="border-t border-[#EAE6DF] pt-6 grid grid-cols-2 gap-4 text-center">
            <div>
              <span className="text-[10px] tracking-wider uppercase font-semibold text-gray-400 block mb-1">
                Pages
              </span>
