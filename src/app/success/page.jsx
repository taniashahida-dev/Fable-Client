...
import { CheckCircle2, ArrowRight, Mail, ArrowLeft, ShoppingBag } from 'lucide-react';
import { db } from '@/lib/mongodb'; 
import { getUserSession } from '@/lib/core/session';

export default async function Success({ searchParams }) {
  // 1. Resolve search params safely matching Next.js structural standards
  ...
  // 2. Fetch specific checkout payload session values from Stripe servers
  ...
  // 3. Fallback redirects if session is unfulfilled or currently incomplete
  if (status === 'open') {
    return redirect('/');
  }
  ...
  if (status === 'complete') {
    try {
      // 4. Synchronize state flags in MongoDB by switching 'pending' records directly to 'completed'
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        ...
        {/* Core Card Outer Shell Interface wrapper mapped to Bright Snow elements */}
        <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center space-y-6">
          
          {/* Visual Success Ring Using New Electric Sapphire Colors (#6366F1) */}
          ...
          <div className="bg-[#F8FAFC] border border-slate-200/80 rounded-xl p-4 text-left space-y-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-[10px] uppercase font-bold text-[#64748B] tracking-wider block">Purchased Item</span>
                <span className="text-sm font-semibold text-[#0F172A] line-clamp-1">{purchasedItemName}</span>
              </div>
            </div>

            {/* Customer Contact Metric */}
            <div className="flex items-start gap-3 border-t border-slate-200/60 pt-2.5">
              <div className="p-2 bg-white border border-slate-200 text-[#0F172A] rounded-lg mt-0.5">
                <Mail className="w-4 h-4 text-[#8B5CF6]" />
              </div>
              <div className="break-all">
                <span className="text-[10px] uppercase font-bold text-[#64748B] tracking-wider block">Sent To</span>
                <span className="text-sm font-medium text-[#0F172A]">{customerEmail}</span>
              </div>
            </div>
          </div>

          {/* Footer Informational Text Block */}
          <p className="text-xs text-[#64748B] leading-relaxed max-w-sm mx-auto">
            ...
            <a href="mailto:support@fable.com" className="text-[#6366F1] hover:underline font-medium">support@fable.com</a>.
          </p>

          {/* Action Call buttons mapped seamlessly to the design guidelines */}
          <div className="pt-2 space-y-2.5">
            
            {/* Primary Navigation Button pointing directly towards the newly finished library shelf view */}
            <Link 
