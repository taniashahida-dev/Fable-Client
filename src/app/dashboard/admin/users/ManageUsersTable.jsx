...
export default function ManageUsersTable({ initialUsers }) {
  const filteredUsers = (initialUsers || []).filter(user => user.role !== "admin");
  ...
  const handleRoleChange = async (id, name, newRole) => {
    if (newRole === "admin") {
      ...
      setIsAdminModalOpen(true); // 👈 এবার হিরোইউআই প্রপ্স অনুযায়ী মোডাল পারফেক্টলি ওপেন হবে
      return;
    }
  ...
  return (
    <>
        ...
        <Table.ScrollContainer>
          <Table.Content aria-label="Manage users table">
            <Table.Header className="bg-[#f8fafc] border-b border-slate-100 text-[#475569] text-[11px] font-bold uppercase tracking-wider">
              <Table.Column isRowHeader className="py-4 px-6 text-left !bg-[#f8fafc] text-slate-500 font-bold">NAME</Table.Column>
            ...
            <Table.Body>
              {users.length === 0 ? (
                <Table.Row>
                  <Table.Cell colSpan="4" className="text-center py-8 text-sm text-gray-500 !bg-white">No readers or writers found.</Table.Cell>
                </Table.Row>
              ) : (
                users.map((user) => (
                  <Table.Row key={user._id} className="border-b border-slate-500/5 hover:bg-[#f8fafc]/60 transition-all duration-200">
                    <Table.Cell className="py-4 px-6 text-sm font-bold text-[#0f172a] !bg-white">{user.name}</Table.Cell>
                    <Table.Cell className="py-4 px-6 text-sm text-slate-600 font-medium !bg-white">{user.email}</Table.Cell>
                    <Table.Cell className="py-4 px-6 text-sm !bg-white">
                      <Chip 
                        size="sm" 
                        variant="flat" 
                        className="capitalize text-[11px] font-bold px-2.5 py-1 rounded-md border-0"
                        color={user.role === "writer" ? "secondary" : "success"}
                      >
                        {user.role}
                      </Chip>
                    </Table.Cell>
                      ...
                      <div className="flex items-center gap-3 justify-center">
                        <Dropdown>
                          ...
                          <Dropdown.Popover className="bg-white border border-slate-100 text-slate-800 rounded-xl shadow-xl">
                            <Dropdown.Menu 
                              aria-label="Roles" 
                              onAction={(key) => handleRoleChange(user._id, user.name, key)}
                              itemClasses={{ 
      ...
      {/* 📋 আলাদা করা অ্যাডমিন কনফার্মেশন মোডাল */}
      <PromoteAdminModal 
