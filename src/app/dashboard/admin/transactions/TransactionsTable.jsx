        ...
        <Table.ScrollContainer className="overflow-x-auto">
          <Table.Content aria-label="All system transactions table">
            ...
            <Table.Header className="bg-[#f8fafc] border-b border-slate-100 text-[#475569] text-[11px] font-bold uppercase tracking-wider">
              <Table.Column
                isRowHeader
                className="py-4 px-6 text-left bg-[#f8fafc]! text-slate-500 font-bold"
              >
                TX ID
              </Table.Column>
              ...
              <Table.Column className="py-4 px-3 md:px-6 text-right bg-[#f8fafc]! text-slate-500 font-bold">
                AMOUNT
              </Table.Column>
              <Table.Column className="py-4 px-3 md:px-6 text-right bg-[#f8fafc]! text-slate-500 font-bold">
                DATE
              ...
              {transactions.map((tx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Table.Cell className="py-3 md:py-4 px-3 md:px-6 text-sm text-slate-600 font-medium bg-white!">
                      #{tx.transactionId}
                    </Table.Cell>
                    ...
                    {/* EBOOK TITLE */}
                    <Table.Cell className="py-3 md:py-4 px-3 md:px-6 text-sm font-bold text-[#0f172a] max-w-30 md:max-w-25 truncate bg-white!">
                      {tx.ebookTitle}
                    </Table.Cell>

                    {/* AMOUNT */}
                    <Table.Cell className="py-3 md:py-4 px-3 md:px-6 text-xs md:text-sm font-bold text-amber-600 text-right bg-white!">
                      ${Number(tx.amount).toFixed(2)}
                    </Table.Cell>

                    {/* DATE */}
                    <Table.Cell className="py-3 md:py-4 px-3 md:px-6 text-xs md:text-sm text-slate-400 font-medium text-right bg-white!">
                      {formatDate(tx.date)}
                    </Table.Cell>
                  </Table.Row>
                );
